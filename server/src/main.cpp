...
// It allows applications to produce web-like interfaces and manipulate them
// directly without the overhead of standard web protocols. It supplies a socket
// which applications connect to. Applications request a container via a key.
// They then do stuff by manipulating html code elements and executing
// javascript.
...
// maps each socket descriptor to its client connection object.
std::map<int, Connection *> activeConnections;
// mutex to lock the activeConnections list during accesses.
std::recursive_mutex mActiveConnections;
...
// CONCURRENCY: this function is only called from within
// Connection::runInstruction which only happens in the main thread with the
// activeConnections mutex held. Therefore there's no need to guard
// mActiveConnections - the lock is already held by this thread.
Container *requestContainerFromKey(std::string key, std::string clientName,
  ...
  if (topLevelKeyList->claimKey(key)) {
    ...
    // not top-level. Traverse each container in case the key refers to a
    // sub-frame.
...
// calls the Connection::service function on each active connection.
// This allows each connection to check if it has queued instructions waiting
// to be executed, and execute them upon the GUI as part of the main thread's
// event loop. RETURNS true if there have been productive service calls made -
// this can be used as a hint to repeat calling this function sooner as this
// is a period of activity.
bool serviceConnections() {
  bool was_productive = false;
  std::lock_guard<std::recursive_mutex> guard(
      mActiveConnections); // lock activeConnections list for access.
...
// exectued in a new thread. responds to incoming socket events.
// new connection request, incoming instruction, etc.
// See: main function, `start execute` section at the end.
void incomingSocketThread() {
  fd_set fds_to_watch;
  // UNCLEAR: purpose of this FD?
  int max_fd;
  // will lock this later to ensure mutual exclusion when accessing
  // activeConnection map.
  std::unique_lock<std::recursive_mutex> guard(mActiveConnections,
                                               std::defer_lock);

  while (true) {
    // fill the fd_set with all the socket descriptors we want to look for data
    // on...
    ...
    for (auto &elmnt : activeConnections) {
      ...
      if (elmnt.first > max_fd)
        max_fd = elmnt.first;
    }
    // done accessing activeConnections for now.
    guard.unlock();

    // block until there's something of interest.
    int numReady = select(++max_fd, &fds_to_watch, NULL, NULL, NULL);
    if (numReady == 0)
      continue; // nothing ready (timeout if applicable)
    if (numReady < 0 /* error */) {
      if (errno == EBADF) {
        // A client may have disconnected but not yet been cleaned up by the
        // service function. we don't know how long until the next service
        // cycle, but there's no point burning CPU cycles to spin this
        // error repeatedly until then. Wait a short time; order of ~10ms.
        usleep(20000);
        continue;
      }
      return; // return since error occur
    }
    ...
    for (auto &elmnt : activeConnections) {
      if (FD_ISSET(elmnt.first, &fds_to_watch)) {
        ...
        // read from this file descriptor into the connection's instruction
        // queue...
...
int main(int argc, char *argv[]) {
  ...
  userid << getuid();
  uid = userid.str();
  // unless overridden by --random argument, default behaviour is used.
  randomDevice = "";
  verbose = true;
  fillscreen = false;
  ...
  brokenImgIcon.loadFromData((const uchar *)brokenImgData,
                             (uint)brokenImgDataLen);
  QWebSettings::globalSettings()->setWebGraphic(
      QWebSettings::MissingImageGraphic, brokenImgIcon);
  QWebSettings::globalSettings()->setWebGraphic(
      QWebSettings::MissingPluginGraphic, brokenImgIcon);

  // Parse command line options ad-hoc to avoid platform dependencies.
  //(QCommandLineParser isn't backwards compatible with Qt4.)
  // store parameters in these vars if specified by user.
  std::string socketFileArg = "";
  std::string keyFileArg = "";
  std::string stylesheetArg = ""; // filename of a CSS file.
  for (int i = 1; i < argc; i++) {
    std::string thisArg = argv[i];
    if (thisArg.compare("--help") == 0 ||
        thisArg.compare("-h") == 0) { // print help text then exit normally.
      std::cout << "Usage: " << argv[0]
                << " [options]\nOptions:\nHipe display server\n\n";
      std::cout << "--css (file path)\n\tLoad a CSS file to provide default "
                   "global styling to all clients.\n";
      std::cout << "--fill\n\tTop level frames fill the entire screen.\n";
      std::cout << "--keyfile (file path)\n\tCreate top-level host key file "
                   "with a custom path and filename.\n";
      std::cout << "--random (file path)\n\tUse a hardware random device (e.g. "
                   "/dev/random), for key generation.\n";
      std::cout << "--silent\n\tOnly critical error messages will be printed "
                   "to the error console.\n";
      std::cout << "--socket (file path)\n\tCreate server socket file with a "
      ...
      if (++i == argc) {
        std::cerr << "Argument not optional. Try '" << argv[0]
      ...
      if (++i == argc) {
        std::cerr << "Argument not optional. Try '" << argv[0]
                  << " --help' for usage information. [--stylesheet ...]\n";
        return 1;
      }
  ...
  signal(SIGPIPE,
         SIG_IGN); // broken client pipes must not crash the display server.
  ...
  // determine default runtime path for the current user.
  char default_path[200];
  default_runtime_dir(default_path, 200);

  ///////
  // initialize `topLevelKeyList` and `keyFilePath`;
  ///////
  std::string keyFile;
  if (keyFileArg.size())
    keyFile = keyFileArg;
  ...
  // 3. map the socket to the location of socketFile
  struct sockaddr_un sockServAddr;
  sockServAddr.sun_family = AF_UNIX;
  strcpy(sockServAddr.sun_path, socketFile.c_str());
  // remove any abandoned socket file of the same name.
  unlink(socketFile.c_str());
  if (bind(serverFD, (struct sockaddr *)&sockServAddr,
           sizeof(struct sockaddr_un)) != 0) {
    std::cerr << "hiped: could not create socket file.\n";
    return 1;
  }

  // 4. make serverFD a 'listening' socket for new connections.
  ...
  // start Qt's event loop, listening for connections, responding to events,
  // etc.
  app.exec();
