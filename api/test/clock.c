...
int main(int argc, char** argv)
{
    ...
    hipe_send(session, HIPE_OPCODE_ADD_STYLE_RULE, 0,0, "body","background-color:black; font-family:arial,sans-serif; color:green;");
    ...
    hipe_send(session, HIPE_OPCODE_SET_STYLE, 0, div_tag, "vertical-align", "top");
    hipe_send(session, HIPE_OPCODE_SET_STYLE, 0, div_tag, "border", "3px inset green");
    ...
    hipe_send(session, HIPE_OPCODE_APPEND_TAG, 0, div_tag, "span", 0);
    hipe_send(session, HIPE_OPCODE_APPEND_TAG, 0, div_tag, "span", 0);
    ...
    while(1) {
        ...
        hipe_send(session, HIPE_OPCODE_SET_TEXT, 0, big_digits, formattedHHMM, 0); 
